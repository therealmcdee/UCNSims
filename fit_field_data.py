...
normalizer[0] = max(data[:,1])
magnitude = np.sqrt(data[:,4]**2 + data[:,5]**2 + data[:,6]**2)
normalizer[1] = max(magnitude)
...
for i in range(len(x)):
    for j in range(len(x)):
        ...
        cnt += 1
print(coefs[plane])
bzfield = generic_zderiv_3(coefs[plane], mesh)
bxfield = generic_xderiv_3(coefs[plane], mesh)
...
for i in range(len(onaxis)):
    ...
    bzrecreate = generic_zderiv_3(coefs[i], plane)
    bzerror[i] = np.sqrt(sum((bzrecreate-data[(data[:,1]==onaxis[i,1])][:,6])**2))
    bxrecreate = generic_xderiv_3(coefs[i], plane)
